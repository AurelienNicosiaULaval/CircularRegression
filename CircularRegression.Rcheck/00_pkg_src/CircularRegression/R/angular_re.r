...
angular_re <- function(
  ...
  } else if (is.data.frame(data) && length(cluster_vec) == nrow(data)) {
    cluster_mf <- cluster_vec[keep_idx]
  } else {
    stop(
      "cluster must have length equal to nrow(data) or to the model-frame size after NA handling.",
      call. = FALSE
    )
  }
  if (anyNA(cluster_mf)) {
    stop("cluster contains missing values after applying model-frame NA handling.", call. = FALSE)
  }

  id <- as.factor(cluster_mf)
  lev <- levels(id)
  m <- length(lev)
...
predict.angular_re <- function(
  ...
  parse_design <- function(formula, data) {
    trm <- stats::delete.response(stats::terms(formula))
    mf <- stats::model.frame(trm, data = data)
    term_labels <- attr(attr(mf, "terms"), "term.labels")
    if (length(term_labels) == 0) {
  ...
  if (is.null(data_source)) {
    stop("No data available for prediction.", call. = FALSE)
  }
